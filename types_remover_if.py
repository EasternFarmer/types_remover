...
def parse_def_statement(line: str) -> str:
	...
	raw_args: list[tuple[str, ...]] = re.findall(
		r'\b([a-zA-Z0-9_*]+):\s\w+|[,(]\s*([a-zA-Z0-9_*]+)\s*[,=]|,\s*([a-zA-Z0-9_*]+)\)',
		line
	)
	args = []
	try:
...
def remove_types(path: str, *, return_str: bool = False, output_file_path: str = 'output.py') -> None | str:
	"""
			...
			match line.split()[0]:
				...
				case _ if line.count('=') == 1:
					if (dict_deep + list_deep + tuple_deep) == 0:
						...
						if variable_name[-1] == ',':
							variable_name = \
								line[0:line.lstrip().index('=') + len(line) - len(line.lstrip())].split(':')[0]
						lst = line.split('=')
						value = lst[1] if len(lst) == 2 else None
						if value is not None:
